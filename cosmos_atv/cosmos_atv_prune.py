#!/usr/bin/env python3
"""Layer-wise ATV pruning for the Cosmos3-Edge Reasoner.

This implementation intentionally supports exactly two pruning protocols:

* joint: a real image+text fused sequence applies official multimodal ATV
  visual-token selection and prunes only the AR language tower.
* separate: a genuinely text-only tokenizer -> embedding -> AR forward applies
  the explicit zero-visual-token ATV ablation and prunes only the AR language
  tower. No vision/projector output is present in calibration.
...
class ActivationStats:
    ...
    def scaler_row(self) -> torch.Tensor:
        if self.nsamples <= 0:
            raise ProtocolError("ATV statistic was requested before any activation samples were collected")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Compatibility guard; official ATV requires this to remain exactly zero.",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    if float(args.vision_sparsity) != 0.0:
        raise ValueError(
            "Official ATV is AR/LLM-only; --vision-sparsity must remain exactly 0. "
    ...
    if not torch.isfinite(torch.tensor(float(args.alpha))).item() or float(args.alpha) < 0.0:
        raise ValueError(f"--alpha must be finite and non-negative, got {args.alpha}")
    if args.nsamples <= 0:
    ...
    if args.protocol == PROTOCOL_JOINT:
        ...
        if args.calibration_json:
            raise ValueError(
                "Separate ATV calibration is text-only; use --ar-calibration-json, "
...
def assert_text_only_record(record: dict[str, Any], context: str) -> None:
    for field in (*IMAGE_FIELDS, *VIDEO_FIELDS):
        ...
        if value not in (None, "", [], {}):
            raise ProtocolError(f"{context}: text-only ATV record contains visual field {field!r}")
    rendered = json.dumps(record, ensure_ascii=False)
    for placeholder in ("<image>", "<video>"):
        if placeholder in rendered:
            raise ProtocolError(f"{context}: text-only ATV record contains {placeholder} placeholder")
...
def build_protocol_calibration(args: argparse.Namespace) -> ProtocolCalibration:
    ...
    for index, (verification_record, ar_record) in enumerate(
        ...
        pair_id = f"{verification_record.dataset}:atv_text_pair:{index}:{pair_digest}"
...
def _flatten_hidden_and_masks(
    ...
    if hidden_states.ndim == 3:
        if hidden_states.shape[0] != 1:
            raise ProtocolError(
                f"{context}: ATV requires calibration batch size 1, got {tuple(hidden_states.shape)}"
            )
...
def run_pruning(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        metadata: dict[str, Any] = {
            ...
            "official_multimodal_atv": args.protocol == PROTOCOL_JOINT,
            "protocol": (
                "cosmos_atv_joint_reasoner"
                if args.protocol == PROTOCOL_JOINT
                else "cosmos_atv_separate_textonly_reasoner"
            ),
            ...
            "alpha": args.alpha,
            "alpha_effective": args.protocol == PROTOCOL_JOINT,
            "visual_token_selection": (
                "atv_cosine_topk" if args.protocol == PROTOCOL_JOINT else "forced_zero"
            ),
            ...
            "atv": {
                ...
                "retained_tokens": (
                    "all valid language tokens plus official top-k visual tokens"
                    if args.protocol == PROTOCOL_JOINT
                    else "all valid language tokens; zero visual tokens"
                ),
            ...
            "execution_order": [
                ...
                "per AR layer: compute ATV token selection, collect retained-token activations, prune Linear weights",
        ...
        ar_reports, ar_masks, _ = prune_ar_layers_atv(
        ...
        metadata["vision_pruning"] = {
            "enabled": False,
            "reason": "official ATV prunes AR/LLM Linear weights only",
            "summary": {
                "layers_pruned": 0,
                "parameters": 0,
                "zeros_after": 0,
        ...
        metadata["layer_statistics"] = [
            report["atv_selection"] for report in ar_reports
        ]
        ...
        if (
            ...
            raise ProtocolError("Vision Linear weights changed during AR-only ATV pruning")
        if (
        ...
        if args.save_masks:
            masks_path = output_dir / "atv_masks.pt"
