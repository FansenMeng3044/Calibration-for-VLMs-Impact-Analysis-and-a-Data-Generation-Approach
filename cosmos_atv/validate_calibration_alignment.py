#!/usr/bin/env python3
"""Assert that joint and text-only ATV protocols use aligned calibration rows."""
...
def main() -> int:
    ...
    for name in ("mmbench", "mmmu", "okvqa"):
        def load(protocol: str):
            ...
            atv.apply_calibration_presets(options)
            return atv.build_protocol_calibration(options)

        joint = load(atv.PROTOCOL_JOINT)
        separate = load(atv.PROTOCOL_SEPARATE)
        joint_text = [atv.normalized_pair_text(record.text) for record in joint.ar_records]
        separate_text = [atv.normalized_pair_text(record.text) for record in separate.ar_records]
        verification_text = [
            atv.normalized_pair_text(record.text)
