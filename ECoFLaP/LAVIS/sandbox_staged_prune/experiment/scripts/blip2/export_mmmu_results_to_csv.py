#!/usr/bin/env python3
"""
Export MMMU single-image eval results (by discipline) for all MMMU-calibrated checkpoints to a CSV.

For each checkpoint matching:
  pruned_checkpoint/okvqa_mmmu_*.pth
this script runs:
  python scripts/blip2/mmmu_eval_by_discipline.py --ckpt CKPT ...
captures the stdout, parses:
  - Overall accuracy
...
Usage (from ECoFLaP/LAVIS):
  ...
  python scripts/blip2/export_mmmu_results_to_csv.py \
    ...
    --device cuda \
    --gpu 1 \
    --outfile mmmu_results_ecoflap.csv
...
def run_eval_for_ckpt(
    ...
    # Respect offline flags if user sets them in the shell.
    proc = subprocess.run(
        cmd,
...
def main():
    parser = argparse.ArgumentParser(
        description="Export MMMU eval (by discipline) for all MMMU-calibrated checkpoints to CSV (ECoFLaP)."
    )
    parser.add_argument(
        "--ckpt_glob",
    ...
    parser.add_argument(
        "--outfile",
        default="mmmu_results_ecoflap.csv",
    ...
    for ckpt in ckpts:
        ...
        # Build one row per discipline, plus overall as a special "discipline"
        rows.append(
            {
                "ckpt": base,
