...
def draw_heatmap(ax, fig, row_labels: Sequence[str], col_labels: Sequence[str], matrix: np.ndarray):
    ...
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_title("Calibration-Evaluation Semantic Similarity", pad=10)
...
def main() -> None:
    ...
    draw_l2_panel(ax_okvqa, okvqa_series, "T5 Decoder L2 Drift on OK-VQA", show_xlabel=True)
    ...
    draw_l2_panel(ax_mmbench, mmbench_series, "T5 Decoder L2 Drift on MMBench", show_xlabel=True)
