...
"""汇总 ECoFLaP 多 benchmark 评测（仅 overall：MMBench/MME/MMMU 来自 jsonl，OKVQA 来自 full val）。"""
...
# (calib_tag, okvqa_fullval_job_id) — 与 run_ecoflap_eval_mme_okvqa_mmmu_calib_ckpts.sh 当前启用的 suite 一致
DEFAULT_SUITES: List[Tuple[str, str]] = [
    ("OKVQA_train_overall_calib", "okvqa_eval_ecoflapOKVQAoverall_fullval"),
    # ("MME_calib", "okvqa_eval_ecoflapMME_fullval"),
    # ("MMMU_overall_calib", "okvqa_eval_ecoflapMMMUoverall_fullval"),
]


def parse_suites_arg(specs: Optional[List[str]]) -> List[Tuple[str, str]]:
    """--suites 每项为 \"calib_tag:okvqa_fullval_job_id\"；不传则用 DEFAULT_SUITES。"""
    if not specs:
...
def build_table(
    ...
    md_lines = [
        "# ECoFLaP 评测汇总（仅 overall）",
        "",
        f"- metrics jsonl: `{metrics_jsonl}`",
        f"- OKVQA full val: `{okvqa_root}/<job_id>/evaluate.txt`",
        "",
        "## 总表（准确率 %）",
        "",
    ...
    md_lines.extend(
        [
            ...
            "- **MMBench / MMMU**：`mmmu_eval_by_discipline.py --overall_only` 的 Overall accuracy。",
            "",
        ]
    )
