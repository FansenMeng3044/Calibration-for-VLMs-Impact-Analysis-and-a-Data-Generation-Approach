#!/usr/bin/env python3
"""Layer-wise SparseGPT pruning for the Cosmos3-Edge Reasoner.

This implementation intentionally supports exactly two pruning protocols:

* joint: image-only Hessians prune the vision encoder first, then a real
  image+text fused sequence prunes the AR tower (matching the existing LLaVA
  VIT-then-LLM execution order).
* separate: image-only Hessians prune the vision encoder, while a genuinely
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--budget-mode", choices=BUDGET_MODES, default=BUDGET_EXACT)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.percdamp < 0:
        raise ValueError("--percdamp cannot be negative")
    if args.max_cholesky_retries < 0:
        raise ValueError("--max-cholesky-retries cannot be negative")
...
def module_audit(model: Cosmos3EdgeForConditionalGeneration) -> dict[str, Any]:
    ...
    vision_parameter_ids = [id(child.weight) for child in vision_linears.values()]
    ar_parameter_ids = [id(child.weight) for child in ar_linears.values()]
    if len(set(vision_parameter_ids)) != len(vision_parameter_ids):
        raise ProtocolError("A vision Linear weight is aliased by multiple target modules")
    if len(set(ar_parameter_ids)) != len(ar_parameter_ids):
        raise ProtocolError("An AR Linear weight is aliased by multiple target modules")
    parameter_id_overlap = set(vision_parameter_ids) & set(ar_parameter_ids)
    if parameter_id_overlap:
        raise ProtocolError("Vision and AR target modules share a Parameter object")
    projector_parameter_ids = {id(parameter) for parameter in reasoner.projector.parameters()}
    if projector_parameter_ids & (set(vision_parameter_ids) | set(ar_parameter_ids)):
        raise ProtocolError("Dense projector shares a Parameter with a SparseGPT target")
    ...
    return {
        ...
        "target_parameter_ids_unique": True,
        "vision_ar_parameter_id_overlap": 0,
...
def prune_layers_layerwise(
    ...
    for layer_index in range(limit):
        ...
        stats = {
            name: SparseGPTStats(module.in_features, device)
        ...
        for name, linear in linears.items():
            ...
            linear_report, mask = sparsegpt_prune_linear(
                linear,
                stats[name],
                sparsity,
                blocksize=blocksize,
                ...
                max_cholesky_retries=max_cholesky_retries,
            )
...
def non_target_zero_report(
    model: Cosmos3EdgeForConditionalGeneration,
) -> dict[str, Any]:
    target_ids = {
        id(linear.weight)
        for layer in model.model.visual.encoder.layers
        for linear in find_linear_modules(layer).values()
    }
    target_ids.update(
        id(linear.weight)
    ...
    with torch.inference_mode():
        for parameter in model.parameters():
            identity = id(parameter)
            if identity in seen or identity in target_ids:
                continue
            seen.add(identity)
            parameters += parameter.numel()
            zeros += int((parameter.data == 0).sum().item())
...
def run_pruning(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        non_target_before = non_target_zero_report(model)
        ...
        metadata: dict[str, Any] = {
            "schema_version": 1,
            "algorithm": "SparseGPT",
            "implementation": "Cosmos3-Edge Reasoner migration of TAMP/LLaVA SparseGPT",
            "protocol": (
                "cosmos_sparsegpt_joint_reasoner"
                if args.protocol == PROTOCOL_JOINT
                else "cosmos_sparsegpt_separate_reasoner"
            ),
            ...
            "sparsegpt": {
                "hessian": "H = 2 / calibration_samples * sum_over_valid_tokens(X X^T)",
                "hessian_token_filter": "valid_tokens; joint AR retains both visual and language tokens",
                "importance": "W^2 / diag(cholesky(inv(H)))^2",
                "reconstruction": "sequential OBS/SparseGPT column error propagation",
                "mask": "per-Linear, per-input-column-block flatten ranking",
            ...
            "model_scope": "reasoner_only",
            "importance_scope": (
                "vision_image_only_then_ar_fused_image_text"
                if args.protocol == PROTOCOL_JOINT
                else "vision_image_only_plus_ar_language_only"
            ),
            "execution_order": [
            ...
            "module_audit": audit,
            "non_target_dense_zero_report_before": non_target_before,
            "started_unix": started_at,
        }
        ...
        metadata["non_target_dense_zero_report_after"] = non_target_zero_report(model)
        if metadata["non_target_dense_zero_report_after"] != non_target_before:
            raise ProtocolError(
                "A non-target Reasoner parameter changed its zero count during SparseGPT"
            )
        ...
        if args.save_masks:
            masks_path = output_dir / "sparsegpt_masks.pt"
