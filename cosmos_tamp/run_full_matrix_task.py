#!/usr/bin/env python3
"""Execute one task from the shared Cosmos TAMP matrix queue."""
...
def validate_checkpoint(
    ...
    command = [
        ...
        "--expected-max-sparsity-per-linear",
        str(config["max_sparsity_per_linear"]),
        "--expected-dtype",
        str(config["dtype"]),
...
def run_prune(config: dict[str, Any], env: dict[str, str]) -> None:
    if float(config["vision_sparsity"]) != 0.0:
        raise RuntimeError("Formal TAMP tasks must keep vision_sparsity exactly zero")
    ...
    preflight = [
        PYTHON_BIN,
        str(CODE_ROOT / "cosmos_tamp_prune.py"),
        "--protocol",
        protocol,
        "--calibration-preset",
        preset,
...
def run_gate(config: dict[str, Any], env: dict[str, str]) -> None:
    """Exercise both TAMP dataflows through save, reload, and all eval adapters."""

    if float(config["vision_sparsity"]) != 0.0:
        raise RuntimeError("TAMP gate must keep vision_sparsity exactly zero")
