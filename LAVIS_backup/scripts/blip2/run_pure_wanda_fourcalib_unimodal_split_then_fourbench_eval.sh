#!/usr/bin/env bash
# =============================================================================
# Pure Wanda unimodal split pruning for question-style calibration datasets.
#
# For each calibration dataset (MMBench / MMMU / OKVQA / MathVista):
#   1. build a text-only JSON from the dataset question/text fields;
#   2. build an image-only CC3M-style JSON from the same dataset images;
#   3. prune T5 with pure Wanda on text only;
#   4. prune ViT with pure Wanda on images only;
#   5. merge the two checkpoints and run MMBench / MMMU / OKVQA / MathVista eval.
#
# This intentionally does NOT pass ECoFLaP-specific score_method,
# sparsity_ratio_granularity, or max_sparsity_per_layer.  It is pure Wanda:
#   abs(weight) * sqrt(input_activation_scaler)
#
# Usage from ECoFLaP/LAVIS:
#   bash scripts/blip2/run_pure_wanda_fourcalib_unimodal_split_then_fourbench_eval.sh
#
# Run one or two datasets:
#   CALIBS="mmbench mmmu" bash scripts/blip2/run_pure_wanda_fourcalib_unimodal_split_then_fourbench_eval.sh
#
# Common overrides:
#   BASE=/data/data2/mfs
#   NUM_DATA=128 PRUNING_CALIB_BATCH=8
#   RUN_EVAL=0
...
if [[ "${PURE_WANDA_FOURCALIB_SPLIT_INNER:-0}" != "1" ]]; then
  if [[ "${CALIB:-}" == "all" ]]; then
    CALIB="mmbench mmmu okvqa mathvista"
  fi
  CALIBS="${CALIBS:-${CALIB:-mmbench mmmu okvqa mathvista}}"
  ...
  if [[ "${#_calibs[@]}" -gt 1 ]]; then
    for _calib in "${_calibs[@]}"; do
      echo ""
      echo "========================================================================"
      echo "== pure Wanda unimodal split calibration: ${_calib}"
      echo "========================================================================"
      CALIB="$_calib" PURE_WANDA_FOURCALIB_SPLIT_INNER=1 bash "$0" "$@"
    done
    echo ""
    echo "========================================================================"
    echo "== all pure Wanda unimodal split calibrations finished: ${CALIBS}"
    echo "========================================================================"
    exit 0
  fi
  export CALIB="${_calibs[0]}"
  export PURE_WANDA_FOURCALIB_SPLIT_INNER=1
fi
...
case "$CALIB" in
  ...
  *)
    echo "[FATAL] CALIB must be one of: mmbench mmmu okvqa mathvista. Got: $CALIB" >&2
    exit 1
    ;;
esac
...
JID_T5="pure_wanda_split_${CALIB_TAG}_text_t5only_${JOB_STAMP}_seed${SEED}"
JID_VIT="pure_wanda_split_${CALIB_TAG}_image_vitonly_${JOB_STAMP}_seed${SEED}"
CKPT_T5="$REPO_ROOT/pruned_checkpoint/${JID_T5}.pth"
CKPT_VIT="$REPO_ROOT/pruned_checkpoint/${JID_VIT}.pth"
MERGED_CKPT="${MERGED_CKPT:-$REPO_ROOT/pruned_checkpoint/merged_pure_wanda_split_${CALIB_TAG}_text_t5_${JID_T5}__image_vit_${JID_VIT}.pth}"

METRICS_JSONL="${LAVIS_METRICS_JSONL:-$REPO_ROOT/lavis/output/BLIP2/pure_wanda_split_${CALIB_TAG}_${JOB_STAMP}_seed${SEED}.jsonl}"
SUMMARY_MD="${SUMMARY_MD:-$REPO_ROOT/lavis/output/BLIP2/pure_wanda_split_${CALIB_TAG}_${JOB_STAMP}_seed${SEED}.md}"
SUMMARY_TSV="${SUMMARY_TSV:-$REPO_ROOT/lavis/output/BLIP2/pure_wanda_split_${CALIB_TAG}_${JOB_STAMP}_seed${SEED}.tsv}"
EVAL_TAG="pure_wanda_split_${CALIB_TAG}_${JOB_STAMP}_seed${SEED}"
...
RUNTIME_T5_CFG="$CFG_WORK_DIR/pure_wanda_split_${CALIB_TAG}_t5_${JOB_STAMP}_seed${SEED}.yaml"
RUNTIME_VIT_CFG="$CFG_WORK_DIR/pure_wanda_split_${CALIB_TAG}_vit_${JOB_STAMP}_seed${SEED}.yaml"
    ...
    for field in fields:
        if field in row:
            ...
            if selected:
                ...
                break
    if not selected:
        raise ValueError("missing text/question field")

    # Preserve common VQA/MMBench context when it exists and has not already
    # been folded into the question by a conversion script.
    parts = [selected]
...
run_prune_t5_text_only() {
  echo ""
  echo ">>> [prune 1/2] pure Wanda | ${CALIB_TAG} text only | prune T5"
  local extra=()
...
run_prune_vit_image_only() {
  echo ""
  echo ">>> [prune 2/2] pure Wanda | ${CALIB_TAG} images only | prune ViT"
...
echo "========== pure Wanda unimodal split: ${CALIB_TAG} text -> T5 + images -> ViT =========="
