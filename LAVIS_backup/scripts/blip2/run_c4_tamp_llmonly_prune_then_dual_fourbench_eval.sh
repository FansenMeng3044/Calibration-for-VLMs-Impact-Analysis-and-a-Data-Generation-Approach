#!/usr/bin/env bash
# =============================================================================
# C4 纯文本标定 → blipt5_tamp_pruner 单模态归约（s = s_l + AMIA）、只剪 T5（llm-only）→ 保存 pth → 四基准评测；
...
echo "========== C4 TAMP 单模态归约 llm-only 剪枝 + 双四基准评测 | STAMP=$JOB_STAMP =========="
...
if [[ "$RUN_PRUNE" == "1" ]]; then
  echo ""
  echo ">>> 剪枝: C4 标定 + TAMP 单模态归约 (s=s_l, AMIA) + 只剪 T5"
  EXTRA=()
