#!/usr/bin/env bash
# =============================================================================
# Hybrid C4 multimodal calibration -> joint pruning -> four-benchmark evaluation.
#
...
#   bash scripts/blip2/run_hybrid_c4_joint_wanda_sparsegpt_eval_fourbench.sh
#
# Common overrides:
#   CALIBS="mmbench cc3m" METHODS="wanda sparsegpt"
#   RUN_EVAL=0
...
METHODS="${METHODS:-wanda sparsegpt}"
...
OUT_ROOT="${OUT_ROOT:-$BASE/hybrid_c4_joint_wanda_sparsegpt_${JOB_STAMP}_seed${SEED}}"
...
method_to_pruner() {
  case "$1" in
    wanda|pure_wanda) echo "blipt5_wanda_pruner" ;;
    sparsegpt) echo "blipt5_sparsegpt_pruner" ;;
...
method_label() {
  case "$1" in
    wanda|pure_wanda) echo "pure_wanda" ;;
    sparsegpt) echo "sparsegpt" ;;
    *) echo "[FATAL] unknown method: $1" >&2; exit 1 ;;
  esac
}
...
echo "========================================================================"
echo "Hybrid C4 multimodal calibration, joint pure Wanda/SparseGPT pruning"
echo "========================================================================"
...
declare -a SUITES=()
METRICS_JSONL="$OUT_ROOT/hybrid_c4_joint_wanda_sparsegpt_fourbench_${JOB_STAMP}_seed${SEED}.jsonl"
: > "$METRICS_JSONL"
...
if [[ "$RUN_EVAL" == "1" ]]; then
  SUMMARY_MD="$SUMMARY_DIR/hybrid_c4_joint_wanda_sparsegpt_fourbench_${JOB_STAMP}_seed${SEED}.md"
  SUMMARY_TSV="$SUMMARY_DIR/hybrid_c4_joint_wanda_sparsegpt_fourbench_${JOB_STAMP}_seed${SEED}.tsv"
