...
"""
汇总 LAVIS_backup 多 benchmark（仅 overall）。
"""
...
DEFAULT_SUITES: List[Tuple[str, str]] = [
    # ("MME_calib", "okvqa_eval_calibMME_fullval"),
    ("OKVQA_train_overall_calib", "okvqa_eval_calibOKVQAoverall_fullval"),
    ("MMMU_overall_calib", "okvqa_eval_calibMMMUoverall_fullval"),
]


def parse_suites_arg(specs: Optional[List[str]]) -> List[Tuple[str, str]]:
    """
    --suites 传入多组 \"calib_tag:okvqa_fullval_job_id\"，与 DEFAULT_SUITES 同形。
    例: --suites 'OKVQA_overall_0324:okvqa_eval_calibOKVQAoverall_0324_fullval'
    """
    if not specs:
    ...
    for s in specs:
        s = s.strip()
        if ":" not in s:
            raise ValueError(f'--suites 项需含冒号 \"TAG:JOB\"，收到: {s!r}')
        tag, job = s.split(":", 1)
        tag, job = tag.strip(), job.strip()
        if not tag or not job:
            raise ValueError(f'--suites 项无效: {s!r}')
        out.append((tag, job))
    return out
...
def build_table(
    ...
    md_lines = [
        ...
        "## 总表（准确率 %）",
        "",
        "| Calibration | MMBench | MME yes/no | OKVQA overall (full val) | MMMU | MathVista MC |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    tsv_lines = [
        "calibration\tmmbench_pct\tmme_yesno_pct\tokvqa_overall_pct\tmmmu_pct\tmathvista_mc_pct"
    ]
    ...
    for calib_tag, fullval_job in suites:
        ...
        mv = picked.get((calib_tag, "MathVista_MC"), {})
        ...
        md_lines.append(
            ...
            f"{fmt(okvqa)} | {fmt(mmmu.get('value'))} | {fmt(mv.get('value'))} |"
        )
        tsv_lines.append(
            "\t".join(
                [
    ...
    md_lines.extend(
        [
            ...
            "- **OKVQA**：全量 val 一次 `evaluate_blip` 的 `agg_metrics`。",
            "- **MathVista MC**：`mathvista_mc_eval.py`（testmini 多选 JSON）。",
            "",
        ]
    )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="TAG:JOB",
        help="覆盖 DEFAULT_SUITES；每项为「校准标签:OKVQA 全量 val 的 job_id」",
    )
    args = ap.parse_args()

    repo = Path(args.repo_root).resolve()
    mj = Path(args.metrics_jsonl).resolve()
    suites = parse_suites_arg(args.suites)
    md_s, tsv_s = build_table(repo, mj, suites)
