...
def load_rows(path: str) -> List[Any]:
    ...
    if lower.endswith(".parquet"):
        try:
            ...
            raise ImportError("Reading parquet annotations requires pandas.") from exc
...
class EncoderForward:
    ...
    def run(
        ...
        result = {
            ...
            "num_query": num_query,
            "seq_len": total,
            # The embeddings that actually enter the T5 (LLM) encoder, kept separate:
            #   visual_tokens   = t5_proj(Q-Former output)   [B, num_query, H]  (changes with pruning)
            #   text_embeddings = embed_tokens(text ids)     [B, T, H]          (unchanged by pruning)
