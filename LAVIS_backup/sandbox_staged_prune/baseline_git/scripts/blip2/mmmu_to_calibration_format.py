...
默认: --out_dir /root/autodl-tmp/MMMU_calibration
  ...
  --by_discipline: 额外按 6 大领域各生成一个 JSON（如 mmmu_calibration_train_Art_Design.json），
      图片仍共用一个 images/ 目录。
  --max_per_discipline N: 与 --by_discipline 合用，每个领域最多取 N 条（dev+validation 合并后顺序取），
      默认 90。用于「每领域一个 calibration、每类 90 条」的剪枝流程。
"""
...
def main():
    ...
    parser.add_argument("--max_per_discipline", type=int, default=0, help="When --by_discipline: max samples per discipline (dev+validation), 0=no limit. Default 90 for 6-class calibration.")
    args = parser.parse_args()
    if args.by_discipline and args.max_per_discipline <= 0:
        args.max_per_discipline = 90
        ...
        for fname in os.listdir(subj_dir):
            ...
            for _, row in df.iterrows():
                ...
                if disc not in by_disc:
                    by_disc[disc] = []
                if args.max_per_discipline <= 0 or len(by_disc[disc]) < args.max_per_discipline:
