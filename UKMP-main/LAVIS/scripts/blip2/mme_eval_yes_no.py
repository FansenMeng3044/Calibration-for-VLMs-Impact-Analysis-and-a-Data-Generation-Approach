...
Compatible with UKMP `ukmp_prune.py` checkpoint format:
  - ukmp_prune.py saves: torch.save({'model': pruned_model_obj}, path)
  - this script loads base model first, then replaces pruned submodules
    to ensure dimensions match for generation.
"""
...
def load_ukmp_pruned_model(args, device: str):
    """
    Load base model and replace pruned submodules when ckpt is UKMP prune .bin.
    """
    ckpt_obj = torch.load(args.ckpt, map_location="cpu")
    if isinstance(ckpt_obj, dict) and "model" in ckpt_obj and isinstance(ckpt_obj["model"], torch.nn.Module):
        pruned_model = ckpt_obj["model"]
        model = load_model("blip2_t5", "pretrain_flant5xl", is_eval=True, device=device)
        if hasattr(model, "visual_encoder") and hasattr(pruned_model, "visual_encoder") and hasattr(pruned_model.visual_encoder, "blocks"):
    ...
    # Fallback: try normal state_dict load.
    return load_model(
        "blip2_t5",
        "pretrain_flant5xl",
        is_eval=True,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MME yes/no eval on parquet (UKMP compatible)")
    ...
    if args.ckpt is None:
        ...
        model = load_ukmp_pruned_model(args, args.device)
    ...
    for i in range(0, len(samples), args.batch_size):
        ...
        with torch.no_grad():
            preds = model.predict_answers(
                ...
                max_len=5,  # UKMP/ECoFLaP wrapper uses this as max_new_tokens
                min_len=1,
