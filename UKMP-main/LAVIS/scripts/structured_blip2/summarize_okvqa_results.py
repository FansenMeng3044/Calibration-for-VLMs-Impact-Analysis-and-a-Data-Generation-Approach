...
# 可选: RESULT_PREFIX=okvqa_fullprecision_eval 只汇总全精度；默认两个都汇总
PREFIXES = os.environ.get("RESULT_PREFIX", "okvqa_eval,okvqa_fullprecision_eval").strip().split(",")
...
def main():
    ...
    for prefix in PREFIXES:
        ...
        if not prefix:
            continue
        results = run_one(prefix)
        title = "全精度 BLIP2" if "fullprecision" in prefix else "剪枝模型 (GHLC/CF 等)"
        print("\n" + "=" * 70)
        print(f"OK-VQA 11 类 — {title} ({prefix})")
        print("=" * 70)
        print(f"{'类别':<6} {'准确率 (%)':<12} {'类别名'}")
        print("-" * 70)
        for label, cat_dir, acc in results:
            if acc is None:
                print(f"{label:<6} {'(缺失)':<12} {cat_dir}")
